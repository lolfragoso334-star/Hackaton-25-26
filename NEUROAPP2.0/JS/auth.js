/* ══════════════════════════════════════════════════════════════════
   auth.js — Autenticación NeuroVida con Supabase Auth nativo
════════════════════════════════════════════════════════════════════ */
...
function requireAuth(rolRequerido = null) {
  ...
  if (rolRequerido && session.rol !== rolRequerido) {
    window.location.replace(session.rol === "admin" ? resolverRutaAdmin() : resolverRutaInicio());
    throw new Error("Rol insuficiente");
  }
  // Restaurar token en memoria para que sbFetch lo use en esta página
...
if (document.getElementById("btn-login")) {
  ...
  if (sessionActiva) {
    window.location.replace(sessionActiva.rol === "admin" ? resolverRutaAdmin() : resolverRutaInicio());
  }
  ...
  document.getElementById("btn-login").addEventListener("click", async () => {
    ...
    window.location.href = resultado.session.rol === "admin" ? resolverRutaAdmin() : resolverRutaInicio();
  });
