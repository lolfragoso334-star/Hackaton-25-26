...
function renderCalMini() {
  ...
  for (let d = 1; d <= diasEnMes; d++) {
    ...
    el.addEventListener('click', () => {
      const tieneCitas = diasConCitas.has(d);
      diaSeleccionado = fechaStr;
      vistaActual = 'dia';

      // Animación de pulso si el día tiene citas
      if (tieneCitas) {
        el.classList.add('animando');
        el.addEventListener('animationend', () => el.classList.remove('animando'), { once: true });
      }

      // Transición del layout: encoge el calendario al seleccionar día con citas
      const layout = document.querySelector('.agenda-layout');
      if (tieneCitas) {
        layout.classList.add('dia-seleccionado');
      } else {
        layout.classList.remove('dia-seleccionado');
      }
...
document.querySelectorAll('.vista-tab').forEach(btn => {
  btn.addEventListener('click', function() {
    ...
    // Restaurar calendario al tamaño grande al cambiar de vista
    document.querySelector('.agenda-layout').classList.remove('dia-seleccionado');
    renderCalMini();
    renderLista();
  });
});
