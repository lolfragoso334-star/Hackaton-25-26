...
async function sbGetNotasClinicas(trabajadorId) {
  // 1) Obtener las notas sin join de FK (evita errores por nombre exacto de FK)
  const { data, error } = await sbFetch(
    `notas_clinicas?trabajador_id=eq.${trabajadorId}&order=created_at.desc&select=id,contenido,created_at,especialista_id`
  );
  if (error) return { data: [], error };
  if (!data || data.length === 0) return { data: [], error: null };

  // 2) Obtener nombres de los especialistas involucrados
  const especialistaIds = [...new Set(data.map(n => n.especialista_id).filter(Boolean))];
  let nombresMap = {};
  if (especialistaIds.length > 0) {
    const { data: perfiles } = await sbFetch(
      `perfiles?id=in.(${especialistaIds.join(",")})&select=id,nombre_completo`
    );
    (perfiles || []).forEach(p => { nombresMap[p.id] = p.nombre_completo; });
  }
  ...
  return {
    data: data.map(n => ({
      ...
      autor:       nombresMap[n.especialista_id] || "Especialista",
    })),
    error: null,
  };
}
